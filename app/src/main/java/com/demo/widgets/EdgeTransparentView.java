...
/**
 * 边缘渐隐容器
 * Created by hengwei on 2021/6/16.
 */
public class EdgeTransparentView extends FrameLayout {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        edgeWidth = typedArray.getDimension(R.styleable.EdgeTransparentView_edge_width,
                ScreenUtil.INSTANCE.convertDpToPx(getContext(), 20));
        typedArray.recycle();
    }
    ...
    private void initShader() {
        mPaint.setShader(new LinearGradient(0, 0, 0, edgeWidth, mGradientColors, mGradientPosition, Shader.TileMode.CLAMP));
    }
    ...
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        ...
        if (position == 0 || (position & topMask) != 0) {
            canvas.drawRect(0, 0, mWidth, edgeWidth, mPaint);
        }
        ...
        if (position == 0 || (position & leftMask) != 0) {
            ...
            canvas.drawRect(0 - offset, 0, mWidth + offset, edgeWidth, mPaint);
